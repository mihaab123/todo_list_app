...
class _TodoListScreenState<T> extends State<TodoListScreen> {
  ...
  List<String> separatorList = ["separator_past".tr(),"separator_today".tr(),"separator_tomorrow".tr(),"separator_next_week".tr(),"separator_future".tr()];
  ...
  int getSeparatorDate(Todo value) {
    ...
    DateTime curDateBeginning= new DateTime(curDate.year, curDate.month, curDate.day);
    DateTime todoDate = DateTime.fromMillisecondsSinceEpoch(value.todoDate);
    DateTime todoDateBeginning= new DateTime(todoDate.year, todoDate.month, todoDate.day);
    ...
    return Scaffold(
        body: Container(
          child: StickyGroupedListView<Todo, int>(
            ...
            groupSeparatorBuilder: (Todo element) =>
                Padding(
  ...
  void completeTodo(Todo todo, int value) {
    if (value == 0) {
      if (todo.repeat.isEmpty) {
        ...
        todo.todoDate = repeatTodo(
            todo.todoDate, todo.repeat);
      }
    } else {
      todo.isFinished = 0;
    }
    ...
    if (todo.isFinished == 0) {
      if (todo.id != null) {
        turnOffNotificationById(
            flutterLocalNotificationsPlugin, todo.id);
      }
      scheduleNotification(
          flutterLocalNotificationsPlugin,
          todo.id.toString(), todo.title,
          DateTime(todo.todoDate));
    } else {
      if (todo.id != null) {
        turnOffNotificationById(
            flutterLocalNotificationsPlugin, todo.id);
    ...
    switch (type) {
      case "h":
        _dateTimeBegin = DateTime(
            _dateTimeBegin.year, _dateTimeBegin.month, _dateTimeBegin.day,
            _dateTimeBegin.hour + count, _dateTimeBegin.minute,
            _dateTimeBegin.second);
        break;
      case "d":
        _dateTimeBegin = DateTime(_dateTimeBegin.year, _dateTimeBegin.month,
            _dateTimeBegin.day + count, _dateTimeBegin.hour,
            _dateTimeBegin.minute, _dateTimeBegin.second);
        break;
      case "w":
        _dateTimeBegin = DateTime(_dateTimeBegin.year, _dateTimeBegin.month,
            _dateTimeBegin.day + (count * 7), _dateTimeBegin.hour,
            _dateTimeBegin.minute, _dateTimeBegin.second);
        break;
      case "m":
        _dateTimeBegin = DateTime(
            _dateTimeBegin.year, _dateTimeBegin.month + count,
            _dateTimeBegin.day, _dateTimeBegin.hour + count,
            _dateTimeBegin.minute, _dateTimeBegin.second);
        break;
      case "y":
        _dateTimeBegin = DateTime(
            _dateTimeBegin.year + count, _dateTimeBegin.month,
            _dateTimeBegin.day, _dateTimeBegin.hour + count,
            _dateTimeBegin.minute, _dateTimeBegin.second);
        break;
    }
          ...
          child: ListTile(
            ...
            onTap: () {
              Navigator.of(context).push(MaterialPageRoute(builder: (context) =>
                  TodoScreen(getTodos: widget.getTodos,
                      todo: todo)));
            },
